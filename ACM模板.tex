...
using namespace std;
const int N = 1001, M = 10001;
int n, m, S, T, u, v, r, c, tot, ans1, ans2;
int Head[N], cur[N], Next[M << 1], Link[M << 1], Rest[M << 1], Cost[M << 1], d[N], From[N], Edge[N];
bool flag[N];
queue<int> q;
...
\end{lstlisting}
\subsection{树链剖分+树状数组}
\begin{lstlisting}
#include <bits/stdc++.h>
using namespace std;
const int N = 100001;
int n, u, v, tot, num;
int a[N], d[N], f[N], s[N], son[N], top[N], idx[N];
int Head[N], Next[N << 1], Link[N << 1];
int sum[N];
inline void AddEdge(int u, int v)
{
    Next[++tot] = Head[u];
...
\end{lstlisting}
\subsection{树链剖分+张昆玮线段树}
\begin{lstlisting}
#include <bits/stdc++.h>
using namespace std;
const int N = 100001;
int n, SIZE, u, v, tot, num;
int a[N], d[N], f[N], s[N], son[N], top[N], idx[N];
int Head[N], Next[N << 1], Link[N << 1];
int sum[N << 2];
inline void AddEdge(int u, int v)
...
const int N = 100001;
int n, u, v, tot, num;
int a[N], d[N], f[N], s[N], son[N], top[N], idx[N], key[N];
int Head[N], Next[N << 1], Link[N << 1];
int l[N << 1], r[N << 1], sum[N << 1], lab[N << 1];
inline void AddEdge(int u, int v)
{
    Next[++tot] = Head[u];
    Link[tot] = v;
